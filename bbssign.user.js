...
(function () {
	if (isURL("bbs.kafan.cn")) {
		...
		//百度文库
		var qdButton = document.getElementsByClassName('g-btn g-btn-pass js-signin-btn g-btn-no')[0];
		var flagText = qdButton.innerHTML;
		if (flagText.indexOf("马上签到") != -1) {
			qdButton.click();
		}
		return;
	}
	else if (isURL("www.horou.com")) {
		//河洛
	...
	else if (isURL("http://bbs.gfan.com/")) { //机锋
		qd();
		if (window.find("签到领奖!")) {
	...
	else if (isURL("http://bbs.ntrqq.net/")) { //NTRQQ
		qd();
		if (window.find("签到领奖!")) {
